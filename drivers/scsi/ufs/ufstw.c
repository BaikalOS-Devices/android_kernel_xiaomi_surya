				     ...
				     bool *flag_res)
{
	...
	/*
	 * Prevent to issue read-lu-flag query request during tw-reset-state.
	 * If not, tw-flag could be cleard, even though the tw-flag was set
	 */
	if (atomic_read(&tw->ufsf->tw_state) != TW_PRESENT) {
		ERR_MSG("tw_state(%d) error",
			atomic_read(&tw->ufsf->tw_state));
		return -ENODEV;
	}
...
void ufstw_suspend(struct ufsf_feature *ufsf)
{
	...
	ret = flush_delayed_work(&ufsf->tw_reset_work);
	TW_DEBUG(ufsf, "flush_delayed_work(tw_reset_work) = %d", ret);
...
static void ufstw_reset(struct ufsf_feature *ufsf)
{
	...
	seq_scan_lu(lun) {
		...
		/*
		 * if tw_reset_work_blocked == true, ufs driver running devfreq jobs,
		 * If so then, defer turbo-write-flag update
		 * until finishing devfreq jobs. It can prevent deadlock
		 * between ufshcd_devfreq_scale and ufsf_query_flag
		 */
		if (ufsf->tw_reset_work_blocked == true) {
			INFO_MSG("retry reset until disabled blocked");
			goto out;
		}
	...
	atomic_set(&ufsf->tw_state, TW_PRESENT);
	INFO_MSG("reset complete.. tw_state %d\n",
			atomic_read(&ufsf->tw_state));
	return;
out:
	/*
	 * If device reset occurs while issuing query request,
	 * ufstw_set_lu_flag() function could be failed.
	 * In this case tw-reset-worker will run again,
	 * so just exit this function.
	 * If not, we will schedule tw-reset-worker explicitly
	 */
	ufstw_lu_put(tw);
	INFO_MSG("reset retry.. tw_state %d work_pending %d",
		 atomic_read(&ufsf->tw_state),
		 delayed_work_pending(&ufsf->tw_reset_work));
	if (!delayed_work_pending(&ufsf->tw_reset_work)) {
		INFO_MSG("schedule tw_reset_work");
		schedule_delayed_work(&ufsf->tw_reset_work, msecs_to_jiffies(500));
	}
}
...
void ufstw_reset_work_fn(struct work_struct *dwork)
{
	struct ufsf_feature *ufsf;
	int ret;

	ufsf = container_of(dwork, struct ufsf_feature, tw_reset_work.work);
