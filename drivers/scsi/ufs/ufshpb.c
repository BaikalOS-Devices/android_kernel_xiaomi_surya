...
{
#if defined(CONFIG_HPB_SUP_ONLY_4)
	if (transfer_len == HPB_4_CHUNK_LEN)
		return true;
#endif
#if defined(CONFIG_HPB_SUP_8_TO_32)
	if (HPB_4_CHUNK_LEN < transfer_len &&
	    transfer_len <= HPB_32_CHUNK_LEN)
		return true;
#endif
#if defined(CONFIG_HPB_SUP_OVER_36)
	if (HPB_32_CHUNK_LEN < transfer_len &&
	    transfer_len <= HPB_512_CHUNK_LEN)
		return true;
#endif
	return false;
}
...
int ufshpb_prepare_add_lrbp(struct ufsf_feature *ufsf, int add_tag)
{
	...
	if (hba != NULL && pre_cmd->request != NULL) {
		if (hba->latency_hist_enabled) {
			switch (req_op(pre_cmd->request)) {
			case REQ_OP_READ:
			case REQ_OP_WRITE:
				pre_cmd->request->lat_hist_io_start =
					ktime_get();
				pre_cmd->request->lat_hist_enabled = 1;
			}
		} else
			pre_cmd->request->lat_hist_enabled = 0;
	}
